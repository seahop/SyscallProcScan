...
HMODULE WINAPI hlpGetModuleHandle(LPCWSTR sModuleName) {
	...
	_PEB_LDR_DATA* Ldr = ProcEnvBlk->Ldr;
	...
	for (LIST_ENTRY* pListEntry = pStartListEntry;  		// start from beginning of InMemoryOrderModuleList
		...
		_LDR_DATA_TABLE_ENTRY* pEntry = (_LDR_DATA_TABLE_ENTRY*)((BYTE*)pListEntry - sizeof(LIST_ENTRY));

		// check if module is found and return its base address
		// changed to _wcsicmp to ignore case and use wide strings
		if (_wcsicmp((const wchar_t*)pEntry->BaseDllName.Buffer, (const wchar_t*)sModuleName) == 0)
